...
//   };
  ...
  const downloadReport = async (sp) => {
    try {
      ...
      doc.setFontSize(18);
      doc.text('Cognitive Assessment Report', 40, 50);
      ...
      const pi = sp.patientInfo.personal_information;
      const mh = sp.patientInfo.medical_history;
      const lines = [
        `Name: ${pi.name || 'N/A'}`,
        `Gender: ${pi.gender || 'N/A'}`,
        `DOB: ${pi.date_of_birth || 'N/A'}`,
        `Education: ${pi.education_level || 'N/A'}`,
        `Memory Issues: ${mh.memory_or_thinking_problems?.response || 'N/A'}`,
        `Balance Problems: ${mh.balance_problems?.response || 'N/A'}`,
        `Mood Status: ${mh.mood_status?.sad_or_depressed || 'N/A'}`,
      ];
      lines.forEach((t) => { doc.text(t, 40, y); y += 18; });

      // Latest Test Summary
      const lt = sp.latestTest;
      y += 10;
      doc.setFont('helvetica', 'bold');
      doc.text('Latest Test Summary', 40, y);
      y += 12;
      doc.setDrawColor(229, 231, 235);
      doc.line(40, y, page.w - 40, y);
      y += 18;
      doc.setFont('helvetica', 'normal');
      if (lt) {
        const prob = typeof lt.prediction?.probability === 'number' ? lt.prediction.probability : null;
        const risk = lt.prediction?.riskLevel || 'N/A';
        const score = lt.totalScore != null ? `${lt.totalScore}/100` : 'N/A';
        doc.text(`Total Score: ${score}`, 40, y); y += 18;
        doc.text(`Risk Level: ${risk}`, 40, y); y += 24;

        // Probability Bar (cool style)
        doc.setFont('helvetica', 'bold');
        doc.text('AI Probability', 40, y);
        doc.setFont('helvetica', 'normal');
        y += 10;
        const barX = 40, barW = page.w - 80, barH = 16; 
        const filledW = prob != null ? Math.max(0, Math.min(1, prob)) * barW : 0;
        // Background
        doc.setFillColor(229, 231, 235);
        doc.roundedRect(barX, y, barW, barH, 8, 8, 'F');
        // Fill
        doc.setFillColor(37, 99, 235);
        doc.roundedRect(barX, y, filledW, barH, 8, 8, 'F');
        doc.setTextColor(17, 17, 17);
        doc.text(prob != null ? `${(prob*100).toFixed(1)}%` : 'N/A', barX + 8, y + barH - 4);
        y += barH + 24;

        // Domain Scores chart (bars)
        doc.setFont('helvetica', 'bold');
        ...
        const domains = [
          ...
          { k: 'orientation', label: 'Orientation' },
        ];
        const chartMax = 100;
        domains.forEach((d) => {
          const val = typeof lt.scores?.[d.k] === 'number' ? Math.max(0, Math.min(chartMax, lt.scores[d.k])) : null;
          const width = val != null ? (val / chartMax) * barW : 0;
          doc.text(d.label, 40, y);
          const cy = y + 6;
          doc.setFillColor(229, 231, 235);
          doc.roundedRect(barX, cy, barW, barH, 8, 8, 'F');
          doc.setFillColor(16, 185, 129);
          doc.roundedRect(barX, cy, width, barH, 8, 8, 'F');
          doc.setTextColor(17, 17, 17);
          doc.text(val != null ? `${val.toFixed(1)}` : 'N/A', barX + barW + 8, cy + barH - 4);
          y += 28;
        });
        ...
        doc.text('Generated by AD Detection App', 40, page.h - 30);
      } else {
        doc.text('No test results available for this patient.', 40, y);
      }

      doc.save(`Report_${pi.name || 'patient'}.pdf`);
    } catch (e) {
  ...
  if (selectedPatient) {
    return (
      <div className="doctor-dashboard-container" style={{ padding: '2rem' }}>
        ...
        <button
          onClick={() => downloadReport(selectedPatient)}
          style={{ marginLeft: '1rem', marginBottom: '1rem', padding: '0.5rem 1rem', backgroundColor: '#111827', color: '#fff', border: 'none', borderRadius: '6px', cursor: 'pointer' }}
        >
          Download Report
        </button>
